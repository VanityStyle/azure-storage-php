...
final class BlobClientTest extends BlobFeatureTestCase
{
    ...
    public function upload_works_with_single_upload(): void
    {
        FileFactory::withStream(1000, function (StreamInterface $file) {
            ...
            $this->blobClient->upload($file, new UploadBlobOptions("text/plain", 2000));
    ...
    public function upload_works_with_parallel_upload(): void
    {
        FileFactory::withStream(1000, function (StreamInterface $file) {
            ...
            $this->blobClient->upload($file, new UploadBlobOptions("text/plain", 500, 100));
    ...
    public function upload_works_with_unknown_sized_stream(): void
    {
        FileFactory::withStream(1000, function (StreamInterface $file) {
            ...
            $this->blobClient->upload($stream, new UploadBlobOptions("text/plain", 500, 100));
    ...
    {
        $this->blobClient->upload("", new UploadBlobOptions("text/plain", 500, 100));
        $properties = $this->blobClient->getProperties();
        self::assertEquals("text/plain", $properties->contentType);
    ...
    public function sync_copy_from_url_works(): void
    {
        ...
        $sourceSas = $sourceBlobClient->generateSasUri(
            BlobSasBuilder::new()
                ->setPermissions(new BlobSasPermissions(true))
    ...
    {
        $sourceContainerClient = $this->serviceClient->getContainerClient($this->randomContainerName());
        $sourceContainerClient->deleteIfExists();
        // cleanup
        $sourceBlobClient = $sourceContainerClient->getBlobClient("to_copy");
        $sourceSas = $sourceBlobClient->generateSasUri(
            BlobSasBuilder::new()
    ...
    public function getting_and_setting_http_headers_works(): void
    {
        ...
        $this->blobClient->upload(
            $compressedContent,
            new UploadBlobOptions(null, 256000000, 8000000, 25, new BlobHttpHeaders(
                "immutable",
                "inline",
                "gzip",
                md5($compressedContent, true),
                "en",
                "text/plain",
            )),
        );
    ...
    public function set_http_headers_works(): void
    {
        ...
        $this->blobClient->upload("test content", new UploadBlobOptions(null, 256000000, 8000000, 25, new BlobHttpHeaders('', '', '', '', '', "application/octet-stream")));
        ...
        $this->blobClient->setHttpHeaders(new BlobHttpHeaders("public, max-age=3600", '', '', '', '', "text/plain"));
    ...
    public function set_http_headers_after_copy_works(): void
    {
        ...
        $sourceBlobClient->upload("content to copy", new UploadBlobOptions(null, 256000000, 8000000, 25, new BlobHttpHeaders('', '', '', '', '', "application/octet-stream")));
        // Copy to target
        ...
        $targetBlobClient->setHttpHeaders(new BlobHttpHeaders('', '', '', '', '', "image/jpeg"));
    ...
    {
        $this->expectException(BlobNotFoundException::class);
        $this->blobClient->setHttpHeaders(new BlobHttpHeaders('', '', '', '', '', "text/plain"));
    }
    ...
    public function set_http_headers_throws_if_container_doesnt_exist(): void
    {
        ...
        $this->serviceClient->getContainerClient("noop")->getBlobClient("noop")->setHttpHeaders(
            new BlobHttpHeaders('', '', '', '', '', "text/plain")
        );
    }
}
